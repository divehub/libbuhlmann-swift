...
final class DecoTests: XCTestCase {
    ...
    func testOxygenSwitchDepthOverridesMODStopIncrement() throws {
        var engine = BuhlmannZHL16C()
        ...
        XCTAssertTrue(
            deco.contains { $0.startDepth == 5 && $0.gas == oxygen },
            "Oxygen should be selected at the configured 5m switch depth even when MOD snaps below it"
        )
    }
    ...
    func testOxygenSwitchDepthDoesNotReplaceLastStopDepth() throws {
        var engine = BuhlmannZHL16C()
        ...
        XCTAssertTrue(
            deco.contains { $0.startDepth == 5 && $0.gas == oxygen },
            "Oxygen should switch at the configured 5m switch depth"
        )
        XCTAssertTrue(
            deco.contains { $0.startDepth == 3 && $0.endDepth == 3 && $0.gas == oxygen },
            "After switching to Oxygen, decompression should continue to the configured 3m last stop"
        )
    }

    func testOxygenSwitchWaypointWaitsForCurrentStopToClear() throws {
        var engine = BuhlmannZHL16C()
        ...
        XCTAssertTrue(
            deco.contains { $0.startDepth == 5 && $0.endDepth == 3 && $0.gas == oxygen },
            "Oxygen should still switch at the configured 5m waypoint"
        )
        XCTAssertFalse(
            deco.contains {
                $0.startDepth == 5 && $0.endDepth == 5
                    && $0.time > 0 && $0.gas == oxygen
            },
            "The 5m switch waypoint must not be treated as clearing the 6m deco stop"
        )
    }

    func testDisabledGasSwitchModeEmitsZeroMinuteSwitchStop() throws {
        var engine = BuhlmannZHL16C()
        ...
        XCTAssertTrue(
            deco.contains { segment in
                segment.startDepth == 5 && segment.endDepth == 5
                    && segment.time == 0 && segment.gas == oxygen
            },
            "Disabled gas switching should emit a zero-minute stop marker on the new gas"
        )
    }
    ...
    func testDoesNotDowngradeFromOxygenToEAN99AfterSwitch() throws {
        ...
        let ean50 = try Gas(o2: 0.50, he: 0.0, maxDepth: 21.755)
        let oxygen = try Gas(o2: 1.0, he: 0.0, maxDepth: 5.837)
        let ean99 = try Gas(o2: 0.99, he: 0.0, maxDepth: 5.955)
        let config = DecoConfig(
            ascentRate: 9,
            surfaceRate: 5,
            stopIncrement: 3,
        ...
        let deco = try engine.calculateDecoStops(
            ...
            currentDepth: 40,
            bottomGas: Gas.air,
            decoGases: [ean50, oxygen, ean99],
            config: config
        )

        guard let firstOxygenIndex = deco.firstIndex(where: { $0.gas == oxygen }) else {
            XCTFail("Expected the schedule to use Oxygen")
            return
        }

        let laterSegments = deco[firstOxygenIndex...]
        XCTAssertFalse(
            laterSegments.contains { $0.gas == ean99 },
            "After switching to Oxygen, the schedule should not downgrade to EAN99"
        )
    }
